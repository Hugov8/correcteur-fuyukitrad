...
def getIdSheets():
    if 'urlSheet' not in request.headers:
        return make_response(jsonify({"erreur": "urlSheet not found"}), 400)
    try:
        wksheet = correcteur.getWorksheet(request.headers['urlSheet'])
    except:
        return make_response(jsonify({"status": "failed", "message": "Vérifier le lien fourni"}), 400)
...
def getCorrectedSheet():
    if 'urlSheet' not in request.headers:
        return make_response(jsonify({"erreur": "Fournir le lien de la sheet"}), 400)
    if 'idSheet' not in request.args:
        return make_response(jsonify({"erreur": "Fournir une id sheet"}), 400)
    try:
        wksheet = correcteur.getWorksheet(request.headers["urlSheet"])
    except:
        return make_response(jsonify({"status":'failed', "message": "Vérifier le lien fourni"}))
    ...
    for row in rows:
        if row.title==request.args['idSheet']:
            try:
                ...
                return make_response(jsonify({"status":"failed", "message":"erreur inconnue"}), 400)
    return make_response(jsonify({"erreur": "Sheet not found"}, 404))
...
def getCorrectedSpreadsheet():
    ...
    try:
        ...
        return make_response(jsonify({"status":"failed", "message":"Vérifier le lien"}))
...
def modifySheet():
    if 'urlSheet' not in request.headers:
        return make_response(jsonify({"erreur": "Fournir le lien de la sheet"}), 400)
    if 'idSheet' not in request.args:
        return make_response(jsonify({"erreur": "Fournir le l'id de la sheet"}), 400)

    data = request.get_json()
    if 'line' not in data and 'value' not in data:
        return make_response(jsonify({"erreur":"Fournir l'attribut line et value dans le corps sous format json"}), 400)
    ...
    for row in rows:
        if row.title==request.args['idSheet']:
            try:
                ...
                return make_response(jsonify({"status":"failed", "message":"Veuillez réessayer plus tard"}), 400)
    return make_response(jsonify({"state": "error, sheet not found"}, 404))
